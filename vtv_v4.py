...
def upload_youtube(link,title,desc,keyword):
    os.chdir('/home/hadn/Downloads/adsense/wowebookpro')
    wget.download(link)
    file_mp4 = link.split("/")[-1]
    cmd = '/home/hadn/python/bin/python /home/hadn/Downloads/adsense/upload_youtube.py --file "%s" --title "%s" --description "%s" --keywords "%s" --category=25' % (file_mp4, title, desc, keyword)
    cmd = shlex.split(cmd.encode('utf8'))
    print cmd
    try:
        up = Popen(cmd, stdout=PIPE)
...
def add_video_to_playlist(videoId, playlistId):
    cmd = '/home/hadn/python/bin/python /home/hadn/Downloads/adsense/wowebookpro/playlist_item.py --videoId %s --playlistId %s' % (videoId, playlistId)
    cmd = shlex.split(cmd)
    print cmd
    try:
        up = Popen(cmd, stdout=PIPE)
...
def upload_youtube1(link):
    #cmd = '/home/hadn/python/bin/python /home/hadn/Downloads/adsense/vtv_detail_v2.py %s' % link
    cmd = '/home/hadn/Downloads/adsense/vtv_detail_v2.sh %s' % link
    cmd = shlex.split(cmd.encode('utf8'))
    print cmd
    try:
        up = Popen(cmd, stdout=PIPE)
...
with open('/home/hadn/Downloads/adsense/vtv.txt') as f:
...
for link in abc_links:
    try:
        ...
        try:
            ...
            print str(e)
        desc = "Website: http://alokidz.com" + desc
